...
<?php
	class ClassApp{
		...
		public function selectSingleClass($id) {
		...
		public function enrollToClass($classId, $studentId) {
			...
			$query = "SELECT * FROM tbl_enroll WHERE classId = '$classId' AND studentId = '$studentId' LIMIT 1";
			$check = $this->db->select($query);
			if ($check != false) {
				$msg = "<div class='alert alert-danger' role='alert'>You already enrolled for this class.</div>";
				return $msg;
			}
			else {
				...
				if ($inserted_row) {
					...
					<a href='classroom.php' class='btn btn-primary'>Go to Classroom</a>
					</div>";
					return $msg;
				} else {
					$msg = "<span class='alert alert-danger'>Enrollment Failed, please try again!</span>";
					return $msg;
				}
			}
		}
		...
		public function selectAllClassesEnrolled($id) {
			$query2 = "SELECT tbl_enroll.*, tbl_class.class_name FROM tbl_enroll INNER JOIN tbl_class ON tbl_enroll.classId = tbl_class.id WHERE tbl_enroll.studentId = '$id' ORDER BY id DESC";
			$getData = $this->db->select($query2);
			return $getData;
		}
	}
?>